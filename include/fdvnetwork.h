...
struct WiFi {
  ...
  static char const *
  convSecurityProtocolToString(SecurityProtocol securityProtocol);
  ...
  // (fatal exception)!
  // channel: 1..13
  static void configureAccessPoint(char const *SSID, char const *securityKey,
                                   uint8_t channel,
                                   SecurityProtocol securityProtocol = WPA2_PSK,
  ...
  static Queue<bool> *getAPInfo(APInfo **infos = NULL, uint32_t *count = NULL,
                                int32_t allocateCount = -1);
};
...
struct IP {
  ...
  static void configureStatic(WiFi::Network network, char const *IP,
                              char const *netmask, char const *gateway);
  ...
  static void getIPInfo(WiFi::Network network, uint8_t *IP, uint8_t *netmask,
                        uint8_t *gateway);
};
...
//    char const* ipaddr = strdup(lookup.get().get_str());   // ipaddr will
//    contain a string like "149.3.176.24" (zero terminated). Resulting string
//    has the same lifetime as NSLookup object.
// Second:
...
public:
  ...
  // ret -1 = error
  int32_t read(void *buffer, uint32_t maxLength, IPAddress *sourceAddress,
               uint16_t *sourcePort);
...
template <typename ConnectionHandler_T, uint16_t MaxThreads_V,
          uint16_t ThreadsStackDepth_V>
class TCPServer {

  static uint32_t const ACCEPTWAITTIMEOUTMS = 20000;
  static uint32_t const SOCKETQUEUESIZE =
      1; // can queue only one socket (nothing to do with working threads)
  ...
  void MTD_FLASHMEM init(uint16_t port) {
    ...
    sLocalAddr.sin_addr.s_addr =
        htonl(INADDR_ANY); // todo: allow other choices other than IP_ADDR_ANY
...
public:
  ...
  void MTD_FLASHMEM listenerTask() {
    while (true) {
      ...
      Socket clientSocket =
          lwip_accept(m_socket, (sockaddr *)&clientAddr, &addrLen);
...
private:
  ...
  void processXWWWFormUrlEncoded(CharChunksIterator headerEnd,
                                 int32_t contentLength);
  void processMultipartFormData(CharChunksIterator headerEnd,
                                int32_t contentLength, char const *contentType);

  CharChunksIterator extractURLEncodedFields(CharChunksIterator begin,
                                             CharChunksIterator end,
                                             Fields *fields);
  CharChunksIterator extractHeaders(CharChunksIterator begin,
                                    CharChunksIterator end, Fields *fields);
...
public:
  ...
  HTTPResponse(HTTPHandler *httpHandler, char const *status,
...
struct ParameterReplacer {
  ...
  void start(char const *strStart, char const *strEnd, Params *params,
...
private:
  ...
  APtr<char>
      m_template; // template file name (filled with the first {%...%} block)
};
...
//   {{#param}}
// Now all parameters like 0param, 1param, 2param, etc.. will be replaced in
// palce of "#param".
//
// A template can only contain {{}} tags, which specify the blocks or parameters
// to replace. Example:
...
//   {%base.html%}                 <- first block specifies the tamplate
//   filename
//   {%the_head%}                  <- start of "the_head" block
//   <title>Home Page</title>
//   {%the_body%}                  <- start of "the_body" block
//   <h1>Hello {{name}}</h1>       <- here the parameter "name" will be replaced
//   with its actual value
//
// Specialized file must contain at least one {%%} tag, and may or may not
// contain {{}} tags (parameters).
// Parameter tags and block tags must have different names.
// The template file can contain parameters that will be replaced as they was in
// the specialized file.
//
...
public:
  ...
  explicit SNTPClient(IPAddress serverIP = IPAddress(193, 204, 114, 232),
                      uint16_t port = 123);
