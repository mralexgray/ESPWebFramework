...
struct CharChunkAllocated : public CharChunkOwn<TYPE_V, SIZE_T> {
  ...
  CharChunkAllocated(SIZE_T capacity_)
      : CharChunkOwn<TYPE_V, SIZE_T>(new char[capacity_], 0),
        capacity(capacity_) {}
} __attribute__((packed));
...
private:
  ...
  uint32_t m_absPos; // absolute position (starting from beginning of
                     // LinkedCharChunks)
...
// LinkedCharChunks
//
// Warn: copy constructor copies only pointers. Should not used with heap or
// stack buffers.
...
struct LinkedCharChunks {
  ...
  LinkedCharChunks(LinkedCharChunks &c) : m_chunks(NULL), m_current(NULL) {
    *this = c;
  }
...
public:
  struct Item {
    ...
    APtr<char> valueStr; // dynamically allocated zero terminated value string
                         // (created by operator[])

    Item(KeyIterator key_, KeyIterator keyEnd_, ValueIterator value_,
         ValueIterator valueEnd_)
        : next(NULL), key(key_), keyEnd(keyEnd_), value(value_),
          valueEnd(valueEnd_) {}
    Item()
        : next(NULL), key(KeyIterator()), keyEnd(KeyIterator()),
          value(ValueIterator()), valueEnd(ValueIterator()) {}
    bool TMTD_FLASHMEM operator==(Item const &rhs) {
      return next == rhs.next && key == rhs.key && keyEnd == rhs.keyEnd &&
             value == rhs.value && valueEnd == rhs.valueEnd;
    }
  ...
  void TMTD_FLASHMEM add(KeyIterator key, KeyIterator keyEnd,
                         ValueIterator value, ValueIterator valueEnd) {
    if (m_items) {
  ...
  // automatically embeds key and value into CharIterator, so key and value can
  // stay in RAM or Flash
  // key and value must terminate with a Zero
  ...
  Item *TMTD_FLASHMEM getItem(char const *key, char const *keyEnd) {
    Item *item = m_items;
    while (item) {
      if (t_compare(item->key, item->keyEnd, CharIterator(key),
                    CharIterator(keyEnd)))
        return item; // found
      item = item->next;
    }
  ...
  // creates a RAM stored temporary (with the same lifetime of IterDict class)
  // zero terminated string with the value content
...
template <typename T> struct ObjectDict {
  ...
  Item *TMTD_FLASHMEM getItem(char const *key, char const *keyEnd) {
    ...
    Item *item = m_items;
    while (item) {
      if (t_compare(CharIterator(item->key), CharIterator(item->keyEnd),
                    CharIterator(key), CharIterator(keyEnd)))
        return item; // found
      item = item->next;
    }
    // not found
...
//
// No need to call eraseContent the first time because it is automatically
// called if MAGIC is not found
...
struct FlashDictionary {
  ...
  static void setValue(char const *key, void const *value,
...
// You can write files into the flash using "binarydir.py" (to prepare) and
// "esptool.py" (to flash) tools.
...
// file.close();    // not really necessary if FlashFile destructor has been
// called
