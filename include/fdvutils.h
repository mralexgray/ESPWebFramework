...
//     if (s_memPool == NULL)
//         s_memPool = new(pvPortMalloc(sizeof(MemPool)))
//         MemPool(pvPortMalloc(FDVHEAPSIZE), FDVHEAPSIZE);
...
private:
  struct MemPoolBlock {
    MemPoolBlock *next;
    SIZE_T size;   // size not including this structure. This is a multiple of
                   // sizeof(MemPoolBlock)
