...
// Flash from 0x0 to 0x8000      mapped at 0x40100000 (RAM), len = 0x8000
// (32KBytes) -> ".text"
// Flash from 0x11000 to 0x6C000 mapped at 0x40211000, len = 0x5B000 (364KBytes)
// -> ".irom.text", ".irom0.text", ".irom1.text", ".irom2.text", ".irom3.text",
// ".irom4.text"
// Flash from 0x6C000 to 0x6D000 mapped at 0x4026C000, len = 0x1000  (4KBytes)
// -> FlashDictionary content
// Flash from 0x6D000 to 0x7B000 mapped at 0x4026D000, len = 0xE000  (56KBytes)
// -> FlashFileSystem content

static uint32_t const FLASH_MAP_START =
    0x40200000; // based on the CPU address space
static uint8_t const *FLASH_MAP_START_PTR = (uint8_t const *)FLASH_MAP_START;

static uint32_t const SDKFLASHSETTINGSZE = 0x5000; // info from look at
                                                   // "eagle.app.v6.ld". Used in
                                                   // getBeginOfSDKSettings()

// Flash address space
static uint32_t const FLASHFILESYSTEM_POS = 0x6D000;
static uint8_t const *FLASHFILESYSTEM_PTR =
    FLASH_MAP_START_PTR + FLASHFILESYSTEM_POS;
...
// Also call enterCritical in the constructor and exitCritical in the destructor
// (if necessary)
