...
// SerialBinary
//
// This binary protocol is bidirectional and equivalent on both sides, there no
// masters and no slaves. Each part can initiate communication.
// Data is enclosed in messages. A message contains an id, a command and
// optional parameters.
// Receiver replies to commands with an ACK message, which can include optional
// parameters.
// Sender must wait up to 2000ms for an ACK message. In case no ACK is received
// it must resend the message with the same ID for 3 times.
// Receiver must ignore a message with the same ID of the previous one.
// Each part should begin sending (or receive) a CMD_READY message before other
// messages.
// All values are little-endian
// Each part should handle the case when it expects an ACK but a command
// arrives. In this case receiver should process the command before, and then
// wait again for the ACK.
// Each part must have the same protocol version.
//
// Example 1. How switch Arduino LED (pin13) on/off. Arduino with "FullControl"
// example sketch.
...
//
// Message structure:
//   1 uint8_t  : Incremental ID. Each part maintain an incremental ID, which
//   identifies a message. Replies include the same ID.
//   1 uint8_t  : Command
...
//     Parameters:
//       1 uint8_t : ID of acknowledged message (different than this ACK message
//       ID!)
...
//     Description:
//       Request a page
//       The ACK may be composed by a standard message plus a content of unknown
//       size terminated by 0x00.
//     CMD Parameters:
...
//       ]
//       1 uint8_t : Number of query fields (/page?field1=xx&field2=yy, has two
...
//                       0 = Embedded in the ACK message: will follow a content
//                       length field plus
//                           actual content data.
//                       1 = Not embedded in the ACK, known size: will follow a
//                       content length field.
//                           Actual data will follow but not embedded in the ACK
//                           message.
//                       2 = Not embedded in the ACK, unknown size: will follow
//                       only actual content
...
//   CMD_STREAMSTART
//     Description:
//       This is a special message that prefix a free stream of data (used for
//       example by the ACK of CMD_HTTPREQUEST)
//     Parameters:
//       none
//     ACK Parameters:
//       none
...
class SerialBinary {
  ...
  static uint8_t const HTTPCONTENTTYPE_UNSPECIFIED =
      0; // header not added. User can still add content-type using addHeader
         // methods.
  static uint8_t const HTTPCONTENTTYPE_TEXTHTML = 1; // text/html (DEFAULT)
  static uint8_t const HTTPCONTENTTYPE_TEXTHTML_UTF8 =
...
public:
  // platforms
  static uint8_t const PLATFORM_BASELINE =
      0; // use when platform specific features aren't used
  ...
  static uint8_t const PIN_CONF_PULLUP =
      0b00000010; // 0 = disabled / 1 = enabled (if supported)
  ...
  struct Message {
    ...
    void MTD_FLASHMEM freeData(); // warn: memory must be explicitly deleted
                                  // using freeData(). Don't create a destructor
                                  // to free data!
  };
