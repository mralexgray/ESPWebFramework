...
// Child class destructor should take care of call terminate() to destroy the
// task.
//
// Example:
//
...
public:
  ...
  void MTD_FLASHMEM setObject(T *object) { m_object = object; }

  void exec() { (m_object->*Method)(); }
...
template <void (*FP)()> void TMTD_FLASHMEM asyncExec_(void *params) {
  FP();
  vTaskDelete(NULL);
}
...
void TMTD_FLASHMEM asyncExec(uint32_t stackDepth = 256, uint32_t priority = 2) {
  xTaskHandle handle;
  xTaskCreate(asyncExec_<FP>, (const signed char *)"", stackDepth, NULL,
              priority, &handle);
}
}
