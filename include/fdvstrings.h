...
// If str or substr is in Chunk use ChunkBuffer<...>::Iterator
template <typename IteratorSTR, typename IteratorSUBSTR>
IteratorSTR TMTD_FLASHMEM t_strstr(IteratorSTR str, IteratorSTR strEnd,
                                   IteratorSUBSTR substr) {
  IteratorSUBSTR b(substr);
  if (*b == 0)
    return str;
  for (; str != strEnd; ++str) {
...
// t_compare
template <typename IteratorS1, typename IteratorS2>
bool TMTD_FLASHMEM t_compare(IteratorS1 s1, IteratorS1 s1End, IteratorS2 s2,
                             IteratorS2 s2End) {
...
// If source is in Chunk use ChunkBuffer<...>::Iterator
template <typename Iterator>
void *TMTD_FLASHMEM t_memcpy(void *destination, Iterator source,
                             uint32_t length) {
...
/////
/////
// String utilities to work with both Flash stored strings and RAM stored
// strings
//
// Example:
