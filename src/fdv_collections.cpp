...
void MTD_FLASHMEM VectorBase::allocate(uint32_t itemsCount) {
  void *newbuf =
      itemsCount > 0 ? Memory::malloc(m_itemSize * itemsCount) : NULL;
  if (m_data) {
...
void MTD_FLASHMEM VectorBase::insert(uint32_t position, void const *item) {
  ...
  memmove(getItem(position + 1), getItem(position),
          m_itemSize * (m_itemsCount - position));
...
CharChunkBase *MTD_FLASHMEM
CharChunkFactory::createCharChunkReference(char *data, uint32_t items) {
  if (items < 256)
...
CharChunkBase *MTD_FLASHMEM
CharChunkFactory::createCharChunkOwn(char *data, uint32_t items) {
  if (items < 256)
...
CharChunkBase *MTD_FLASHMEM
CharChunkFactory::createCharChunkAllocated(uint32_t capacity) {
  if (capacity < 256)
...
CharChunkBase *MTD_FLASHMEM
CharChunkFactory::createCharChunkLink(LinkedCharChunks *link) {
...
MTD_FLASHMEM CharChunksIterator::CharChunksIterator(CharChunksIterator const &c)
    : m_chunk(c.m_chunk), m_pos(c.m_pos), m_absPos(c.m_absPos),
      m_linkedNext(c.m_linkedNext) {
  checkLinkedChunks();
}

CharChunksIterator &MTD_FLASHMEM CharChunksIterator::
operator=(CharChunksIterator const &c) {
  m_chunk = c.m_chunk;
  m_pos = c.m_pos;
  m_absPos = c.m_absPos;
  m_linkedNext = c.m_linkedNext;
...
bool MTD_FLASHMEM CharChunksIterator::
operator==(CharChunksIterator const &rhs) {
...
// LinkedCharChunks
//
// Warn: copy constructor copies only pointers. Should not used when with heap
// or stack buffers.
...
CharChunkBase *MTD_FLASHMEM LinkedCharChunks::addChunk(char *data,
                                                       uint32_t items,
                                                       bool freeOnDestroy) {
  return addChunk(
      freeOnDestroy ? CharChunkFactory::createCharChunkOwn(data, items)
                    : CharChunkFactory::createCharChunkReference(data, items));
}

// const to non const cast
CharChunkBase *MTD_FLASHMEM LinkedCharChunks::addChunk(char const *data,
                                                       uint32_t items,
                                                       bool freeOnDestroy) {
  return addChunk((char *)data, items, freeOnDestroy);
}

void MTD_FLASHMEM LinkedCharChunks::addChunk(char const *str,
                                             bool freeOnDestroy) {
  addChunk(str, f_strlen(str),
           freeOnDestroy); // "items" field doesn't include ending zero
}
...
void MTD_FLASHMEM LinkedCharChunks::dumpChunks()
{
    ...
    while (chunk)
    {
        debug(FSTR("this = %p type = %d next = %p data = %p items = %d"), this,
chunk->type, chunk->next, chunk->data, chunk->getItems());
...
// Other threads should not call any FlashFileSystem method while one thread is
// inside "remove"
...
MTD_FLASHMEM void FlashFile::create(char const *filename,
                                    char const *mimetype) {
  ...
  // write flags
  uint8_t flags =
      0x01; // bit0 = 1 (end of files), until the file has been closed
  ...
  m_headerLength = sizeof(flags) + sizeof(filenamelen) + sizeof(mimetypelen) +
                   sizeof(filelen) + filenamelen + mimetypelen;
}
...
// clear the entire available space and write MAGIC at the beginning of the
// dictionary
...
void STC_FLASHMEM FlashDictionary::setValue(char const *key, void const *value,
  ...
  // memcpy is ok, because source alredy aligned then can be read directly from
  // Flash
  uint8_t *page = new uint8_t[SPI_FLASH_SEC_SIZE];
  memcpy(page, (void const *)(FLASH_MAP_START + FLASH_DICTIONARY_POS),
         SPI_FLASH_SEC_SIZE);

  // get key position as index into page[]
  uint32_t keyPos =
      (uint32_t)keyPosPtr - FLASH_MAP_START - FLASH_DICTIONARY_POS;
  ...
  if (!isNewKey) {
    uint32_t prevValueLength =
        page[curPos] | (page[curPos + 1] << 8); // little-endian
...
uint8_t const *STC_FLASHMEM FlashDictionary::getValue(char const *key,
                                                      uint32_t *valueLength) {
...
void STC_FLASHMEM FlashDictionary::setString(char const *key,
                                             char const *value) {
  if (value)
    setValue(key, value, f_strlen(value) + 1);
}

// always returns a Flash stored string (if not found return what contained in
// defaultValue)
char const *STC_FLASHMEM FlashDictionary::getString(char const *key,
...
int32_t STC_FLASHMEM FlashDictionary::getInt(char const *key,
...
// what is stored:
//   byte: key length (or 0xFF for end of keys, hence starting of free space).
//   Doesn't include ending zero
//   ....: key string
...
void const *MTD_FLASHMEM FlashDictionary::findKey(char const *key) {
  if (!isContentValid())
    eraseContent();
  uint8_t const *curpos =
      (uint8_t const *)(FLASH_MAP_START + FLASH_DICTIONARY_POS + sizeof(MAGIC));
  while (true) {
    uint8_t keylen = getByte(curpos); // keylen doesn't include ending zero
    if (keylen == 0xFF ||
        (key && f_strcmp((char const *)(curpos + 1), key) == 0))
...
void MTD_FLASHMEM StringList::insert(uint32_t position, char const *string,
                                     Storage storage) {
  m_items.insert(position,
                 Item(storage, storage == Heap ? f_strdup(string) : string));
}
