...
IPAddress DateTime::s_defaultNTPServer; // never initialize (no constructor is
                                        // called here!)
bool DateTime::s_synchingWithNTPServer = false;

// a local copy of defaultNTPServer string is perfomed
void MTD_FLASHMEM DateTime::setDefaults(int8_t timezoneHours,
...
bool MTD_FLASHMEM DateTime::getFromNTPServer(IPAddress const &serverIP) {
  IPAddress ip =
      (serverIP == IPAddress(0, 0, 0, 0) ? s_defaultNTPServer : serverIP);
...
// if this is the first time calling now() or after 6 hours an NTP update is
// perfomed
...
DateTime MTD_FLASHMEM DateTime::now() {
  ...
  uint32_t diff = (currentMillis < locLastMillis)
                      ? (0xFFFFFFFF - locLastMillis + currentMillis)
                      : (currentMillis - locLastMillis);
  ...
  if (s_defaultNTPServer == IPAddress(0, 0, 0, 0)) {
    ...
    {
      // reset millis counter to avoid overflow (actually it overflows after 50
      // days)
...
DateTime MTD_FLASHMEM DateTime::lastSyncDateTime(bool set,
  ...
  if (s_lastDateTime.year > 2000 &&
      ConfigurationManager::getBootDateTime().year == 2000) {
...
char const *MTD_FLASHMEM DateTime::decode(char const *inbuf,
...
//    '%w' : Numeric representation of the day of the week (0 = Sunday, 6 =
//    Saturday)
//    '%d' : Day of the month, 2 digits with leading zeros (01..31)
...
uint16_t MTD_FLASHMEM DateTime::format(char *outbuf, char const *formatstr) {
  static char const *DAYS[] = {
      FSTR("Sunday"),   FSTR("Monday"), FSTR("Tuesday"), FSTR("Wednesday"),
      FSTR("Thursday"), FSTR("Friday"), FSTR("Saturday")};
  static char const *MONTHS[] = {
      FSTR("January"),   FSTR("February"), FSTR("March"),    FSTR("April"),
  ...
  for (; getChar(formatstr); ++formatstr) {
    if (getChar(formatstr) == '%') {
      ...
      case 'H':
        ...
        break;
      case 'I':
        outbuf += sprintf(outbuf, FSTR("%02d"),
                          (hours == 0 || hours == 12) ? 12 : (hours % 12));
        break;
      case 'p':
        outbuf +=
            sprintf(outbuf, FSTR("%s"), hours > 11 ? FSTR("PM") : FSTR("AM"));
        break;
      case 'M':
      ...
      case 'z':
        outbuf +=
            sprintf(outbuf, FSTR("%+03d%02d"), timezoneHours, timezoneMinutes);
        break;
      case 'j':
...
uint8_t MTD_FLASHMEM DateTime::daysInMonth(uint8_t month) {
  static uint8_t const DIMO[] = {31, 28, 31, 30, 31, 30,
                                 31, 31, 30, 31, 30, 31};
  return DIMO[month];
}

long MTD_FLASHMEM DateTime::time2long(uint16_t days, uint8_t h, uint8_t m,
                                      uint8_t s) {
