...
uint32_t FUNC_FLASHMEM getActualFlashSize() {
  ...
  if (f_memcmp(FLASH_MAP_START_PTR, FLASH_MAP_START_PTR + 0x500000, COMPSIZE) !=
      0) {
    // does not overlap at 0x0, should be 4MB (32mbit)
    return 0x400000;
  }

  if (f_memcmp(FLASH_MAP_START_PTR, FLASH_MAP_START_PTR + 0x300000, COMPSIZE) !=
      0) {
    // does not overlap at 0x0, should be 2MB (16mbit)
    return 0x200000;
  }

  if (f_memcmp(FLASH_MAP_START_PTR, FLASH_MAP_START_PTR + 0x80000, COMPSIZE) !=
      0) {
...
// Actually flash memory can be mapped a 1MB a the time starting from
// FLASH_MAP_START
// This function creates a "virtual" allocation from FLASH_MAP_START up to the
// actual size of the flash memory,
...
uint8_t FUNC_FLASHMEM getByte(void const *address) {
  if (isStoredInFlash(address)) {
    ...
    uint32_t const *alignedAddress =
        (uint32_t const *)((uint32_t)address & 0xFFFFFFFC);
...
uint32_t FUNC_FLASHMEM getDWord(void const *buffer) {
  uint8_t const *pc = (uint8_t const *)buffer;
  return getByte(pc) | (getByte(pc + 1) << 8) | (getByte(pc + 2) << 16) |
         (getByte(pc + 3) << 24);
}
...
void ICACHE_FLASH_ATTR FlashWriter::loadPage() {
  // alignedDest is aligned to the start of flash page (a page is
  // SPI_FLASH_SEC_SIZE bytes)
  uint8_t const *alignedDest =
      (uint8_t const *)((uint32_t)m_dest & ~(SPI_FLASH_SEC_SIZE - 1));
  if (alignedDest != m_currentPage) {
    ...
    void const *addr =
        FLASH_MAP_START_PTR + ((uint32_t)m_currentPage &
                               0xFFFFF); // calculate actual address (only first
                                         // megabyte is directly addressable)
...
bool MTD_FLASHMEM FlashWriter::write(void const *source, uint32_t size) {
  for (uint8_t const *bSrc = (uint8_t const *)source; size > 0;
       --size, ++m_dest) {
