...
Cmd const *FUNC_FLASHMEM cmdInfo(uint32_t i) {
  static const Cmd cmds[] = {
      ...
      //   help
      //   help ifconfig
      {FSTR("help"), FSTR("[COMMAND]"), FSTR("Show help"),
      ...
      //   reboot 500
      {FSTR("reboot"), FSTR("[MS]"),
       FSTR("Restart system in [MS] milliseconds"), &SerialConsole::cmd_reboot},

      {FSTR("restore"), STR_, FSTR("Erase Flash stored settings"),
       &SerialConsole::cmd_restore},

      {FSTR("free"), STR_, FSTR("Display amount of free and used memory"),
      ...
      {FSTR("ifconfig"), FSTR("[static IP NETMASK GATEWAY] | [dhcp] | [ap IP "
                              "NETMASK GATEWAY] | [dns DNS1 DNS2]"),
       FSTR("No params: Display network info\r\n\tstatic: Set Client Mode "
      ...
      //   iwlist
      //   iwlist scan
      {FSTR("iwlist"), FSTR("[scan]"),
       FSTR("Display or scan for available wireless networks"),
       &SerialConsole::cmd_iwlist},

      {STR_date, STR_, FSTR("Display current date/time"),
       &SerialConsole::cmd_date},

      // example:
      //   ntpdate 192.168.1.10
      //   ntpdate ntp1.inrim.it
      {FSTR("ntpdate"), FSTR("[SERVER]"),
       FSTR("Display date/time from NTP server"), &SerialConsole::cmd_ntpdate},

      // example:
      //   nslookup www.google.com
      {FSTR("nslookup"), FSTR("NAME"), FSTR("Query DNS"),
       &SerialConsole::cmd_nslookup},

      {STR_uptime, STR_, FSTR("Display how long the system has been running"),
       &SerialConsole::cmd_uptime},

      {FSTR("ping"), FSTR("SERVER"),
      ...
      //   ls
      //   ls -l
      {STR_ls, FSTR("[-l]"), FSTR("List flash directory contents"),
      ...
      // example:
      //   cat test.html
      {STR_cat, FSTR("FILENAME"), FSTR("Print a file"),
       &SerialConsole::cmd_cat},

      // example:
      //   fixflash
      {FSTR("fixflash"), STR_, FSTR("Fixes flash size to the detected value"),
...
bool MTD_FLASHMEM SerialConsole::hasParameter(uint32_t paramIndex,
                                              char const *str) {
...
void MTD_FLASHMEM SerialConsole::cmd_ifconfig() {
  if (m_paramsCount == 5 && hasParameter(1, FSTR("static"))) {
    ...
    ConfigurationManager::setClientIPParams(true, strIP.get(), strMSK.get(),
                                            strGTY.get());
    ...
    APtr<char> strMSK(t_strdup(m_params[3]));
    APtr<char> strGTY(t_strdup(m_params[4]));
    ConfigurationManager::setAccessPointIPParams(strIP.get(), strMSK.get(),
                                                 strGTY.get());
    ...
    ConfigurationManager::setDNSParams(IPAddress(strDNS1.get()),
                                       IPAddress(strDNS2.get()));
    ...
    for (int32_t i = 0; i < 2; ++i) {
      ...
      m_serial->printf(FSTR("   inet %d.%d.%d.%d netmask %d.%d.%d.%d gateway "
                            "%d.%d.%d.%d\r\n"),
                       IP[0], IP[1], IP[2], IP[3], netmask[0], netmask[1],
                       netmask[2], netmask[3], gateway[0], gateway[1],
                       gateway[2], gateway[3]);
      if (i == 0) {
...
void MTD_FLASHMEM SerialConsole::cmd_iwconfig() {
  if (m_paramsCount == 4 && hasParameter(1, FSTR("client"))) {
    ...
    ConfigurationManager::setAccessPointParams(
        strSSID.get(), strPSW.get(), intCHANNEL, WiFi::WPA2_PSK, false);
  } else {
    // show info
    ...
    if (mode == WiFi::AccessPoint || mode == WiFi::ClientAndAccessPoint) {
      ...
      bool hiddenSSID;
      ConfigurationManager::getAccessPointParams(
          &SSID, &psw, &channel, &securityProtocol, &hiddenSSID);
      m_serial->printf(FSTR("   SSID: %s\r\n"), SSID);
      m_serial->printf(FSTR("   PSW : %s\r\n"), psw);
...
void MTD_FLASHMEM SerialConsole::cmd_iwlist() {
  ...
  for (uint32_t i = 0; i != count; ++i) {
    m_serial->printf(FSTR("  %2d - Address: %02X:%02X:%02X:%02X:%02X:%02X\r\n"),
                     i, infos[i].BSSID[0], infos[i].BSSID[1], infos[i].BSSID[2],
                     infos[i].BSSID[3], infos[i].BSSID[4], infos[i].BSSID[5]);
    m_serial->printf(FSTR("       SSID: %s\r\n"), infos[i].SSID);
...
void MTD_FLASHMEM SerialConsole::cmd_ping() {
  ...
  while (true) {
    ...
    m_serial->printf(
        FSTR("%d bytes from %s: icmp_seq=%d ttl=%d time=%.3f ms\r\n"),
        icmp.receivedBytes(), (char const *)serverIP.get_str(),
        icmp.receivedSeq(), icmp.receivedTTL(), r);
...
MTD_FLASHMEM SerialBinary::Message::Message(uint8_t ID_, uint8_t command_,
                                            uint16_t dataSize_)
    : valid(true), ID(ID_), command(command_), dataSize(dataSize_),
      data(dataSize_ ? new uint8_t[dataSize_] : NULL) {}

MTD_FLASHMEM SerialBinary::Message::Message(uint8_t ID_, uint8_t command_,
                                            uint8_t *data_, uint16_t dataSize_)
    : valid(true), ID(ID_), command(command_), data(data_),
      dataSize(dataSize_) {}
...
MTD_FLASHMEM SerialBinary::SerialBinary()
    : m_serial(HardwareSerial::getSerial(0)), m_recvID(255), m_sendID(0),
      m_recvACKQueue(ACKMSG_QUEUE_LENGTH), m_receiveTask(this, false, 256),
      m_isReady(false), m_platform(PLATFORM_BASELINE), m_HTTPRoutes(NULL) {
  checkReady();
}
...
SerialBinary::Message MTD_FLASHMEM SerialBinary::receive() {
  ...
  while (!timeout) {
    ...
    if (msg.dataSize > 0 && msg.dataSize < (Task::getFreeHeap() >> 1)) {
      ...
      if (m_serial->read(msg.data, msg.dataSize, INTRA_MSG_TIMEOUT) <
          msg.dataSize) {
        msg.freeData();
        continue;
      }
  ...
  while (true) {
    ...
    if (msg.valid) {
      if (msg.command == CMD_ACK)
        // if message is an ACK then put it into the ACK message queue, another
        // task will handle it
...
void MTD_FLASHMEM SerialBinary::handle_CMD_READY(Message *msg) {
  ...
  m_isReady = (protocolVersion == PROTOCOL_VERSION &&
               f_strcmp(magicString, STR_BINPRORDY) == 0);
  m_mutex.unlock();
...
bool MTD_FLASHMEM SerialBinary::send_CMD_HTTPREQUEST(uint8_t pageIndex,
                                                     HTTPHandler *handler) {
  MutexLock lock(&m_himutex);
  if (checkReady()) {
    for (uint32_t i = 0; i != MAX_RESEND_COUNT; ++i) {
      ...
      uint32_t msglen = 5; // (1) method, (1) page index, (1) headers fields
                           // count, (1) query fields count, (1) form fields
                           // count
