...
void STC_FLASHMEM ConfigurationManager::applyWiFi() {
  ...
  if (getWiFiMode() == WiFi::AccessPoint ||
      getWiFiMode() == WiFi::ClientAndAccessPoint) {
    ...
    bool hiddenSSID;
    getAccessPointParams(&SSID, &securityKey, &channel, &securityProtocol,
                         &hiddenSSID);
    WiFi::configureAccessPoint(SSID, securityKey, channel, securityProtocol,
                               hiddenSSID);
  }

  if (getWiFiMode() == WiFi::Client ||
      getWiFiMode() == WiFi::ClientAndAccessPoint) {
    // Client parameters
    char const *SSID;
    char const *securityKey;
...
WiFi::Mode STC_FLASHMEM ConfigurationManager::getWiFiMode() {
  return (WiFi::Mode)FlashDictionary::getInt(STR_WiFiMode,
                                             (int32_t)WiFi::AccessPoint);
}

void STC_FLASHMEM ConfigurationManager::setAccessPointParams(
    char const *SSID, char const *securityKey, uint8_t channel,
...
void STC_FLASHMEM ConfigurationManager::getAccessPointParams(
    char const **SSID, char const **securityKey, uint8_t *channel,
    WiFi::SecurityProtocol *securityProtocol, bool *hiddenSSID) {
  static char defaultSSID[10];
  uint8_t mac[16];
  WiFi::getMACAddress(WiFi::AccessPointNetwork, mac);
  sprintf(defaultSSID, FSTR("ESP%02X%02X%02X%02X%02X%02X"), mac[0], mac[1],
  ...
  *securityProtocol = (WiFi::SecurityProtocol)FlashDictionary::getInt(
      STR_APSP, (int32_t)WiFi::Open);
  *hiddenSSID = FlashDictionary::getBool(STR_APHSSID, false);
}

void STC_FLASHMEM ConfigurationManager::setClientParams(
    char const *SSID, char const *securityKey) {
  FlashDictionary::setString(STR_CLSSID, SSID);
  FlashDictionary::setString(STR_CLSECKEY, securityKey);
}

void STC_FLASHMEM ConfigurationManager::getClientParams(
    char const **SSID, char const **securityKey) {
...
void STC_FLASHMEM ConfigurationManager::setClientIPParams(bool staticIP,
                                                          char const *IP,
                                                          char const *netmask,
                                                          char const *gateway) {
...
// IP, netmask, gateway valid only if staticIP = true
void STC_FLASHMEM ConfigurationManager::getClientIPParams(
    bool *staticIP, char const **IP, char const **netmask,
    char const **gateway) {
...
void STC_FLASHMEM ConfigurationManager::setAccessPointIPParams(
    char const *IP, char const *netmask, char const *gateway) {
...
void STC_FLASHMEM ConfigurationManager::getAccessPointIPParams(
    char const **IP, char const **netmask, char const **gateway) {
...
void STC_FLASHMEM ConfigurationManager::setDHCPServerParams(bool enabled,
                                                            char const *startIP,
                                                            char const *endIP) {
...
void STC_FLASHMEM ConfigurationManager::getDHCPServerParams(
    bool *enabled, char const **startIP, char const **endIP) {
...
void STC_FLASHMEM ConfigurationManager::setDNSParams(IPAddress DNS1,
                                                     IPAddress DNS2) {
  FlashDictionary::setInt(STR_DNS1, DNS1.get_uint32());
  FlashDictionary::setInt(STR_DNS2, DNS2.get_uint32());
}

void STC_FLASHMEM ConfigurationManager::getDNSParams(IPAddress *DNS1,
                                                     IPAddress *DNS2) {
...
void STC_FLASHMEM ConfigurationManager::setUARTParams(
    uint32_t baudRate, bool enableSystemOutput, SerialService serialService) {
...
void STC_FLASHMEM ConfigurationManager::getUARTParams(
    uint32_t *baudRate, bool *enableSystemOutput,
    SerialService *serialService) {
  *baudRate = FlashDictionary::getInt(STR_BAUD, 9600);
  *enableSystemOutput = FlashDictionary::getBool(STR_SYSOUT, false);

#if (FDV_INCLUDE_SERIALCONSOLE == 1)
  *serialService = (SerialService)FlashDictionary::getInt(
      STR_UARTSRV, (int32_t)SerialService_Console);
#elif(FDV_INCLUDE_SERIALBINARY == 1)
  *serialService = (SerialService)FlashDictionary::getInt(
      STR_UARTSRV, (int32_t)SerialService_BinaryProtocol);
#else
  *serialService = (SerialService)FlashDictionary::getInt(
      STR_UARTSRV, (int32_t)SerialService_None);
#endif
}
...
void STC_FLASHMEM ConfigurationManager::setGPIOParams(uint32_t gpioNum,
                                                      bool configured,
                                                      bool isOutput,
                                                      bool pullUp, bool value) {
...
void STC_FLASHMEM ConfigurationManager::getGPIOParams(uint32_t gpioNum,
                                                      bool *configured,
                                                      bool *isOutput,
                                                      bool *pullUp,
                                                      bool *value) {
...
void STC_FLASHMEM ConfigurationManager::setDateTimeParams(
    int8_t timezoneHours, uint8_t timezoneMinutes,
    char const *defaultNTPServer) {
...
void STC_FLASHMEM ConfigurationManager::getDateTimeParams(
    int8_t *timezoneHours, uint8_t *timezoneMinutes,
    char const **defaultNTPServer) {
  *timezoneHours = FlashDictionary::getInt(STR_TZHH, 0);
  *timezoneMinutes = FlashDictionary::getInt(STR_TZMM, 0);
  *defaultNTPServer =
      FlashDictionary::getString(STR_DEFNTPSRV, FSTR("ntp1.inrim.it"));
}

DateTime STC_FLASHMEM
ConfigurationManager::getBootDateTime(bool set, DateTime const &value) {
  static DateTime s_bootTime;
  static bool s_init = false;

  if (!s_init) {
    // this is necessary because statics classes (like DateTime is not
    // initializated automatically)
    s_init = true;
    s_bootTime = DateTime();
  }
...
void MTD_FLASHMEM
HTTPHelperConfiguration::getWiFiMode(HTTPTemplateResponse *response) {
...
void MTD_FLASHMEM
HTTPHelperConfiguration::setWiFiMode(HTTPTemplateResponse *response) {
...
void MTD_FLASHMEM HTTPHelperConfiguration::getClientModeWiFiParams(
    HTTPTemplateResponse *response) {
  char const *SSID;
  char const *securityKey;
...
void MTD_FLASHMEM HTTPHelperConfiguration::setClientModeWiFiParams(
    HTTPTemplateResponse *response) {
  ConfigurationManager::setClientParams(
      response->getRequest().form[FSTR("CLSSID")],
      response->getRequest().form[FSTR("CLPSW")]);
}

void MTD_FLASHMEM
HTTPHelperConfiguration::getClientModeIP(HTTPTemplateResponse *response) {
  bool staticIP;
  char const *IP;
  char const *netmask;
  char const *gateway;
...
void MTD_FLASHMEM
HTTPHelperConfiguration::setClientModeIP(HTTPTemplateResponse *response) {
  ConfigurationManager::setClientIPParams(
      f_strcmp(response->getRequest().form[STR_CLIPMOD], STR_stcIP) == 0,
      response->getRequest().form[STR_CLIP],
      response->getRequest().form[STR_CLMSK],
      response->getRequest().form[STR_CLGTW]);
}

// DNS1str and DNS2str cannot be allocated into the stack here because they are
// used in response object
void MTD_FLASHMEM HTTPHelperConfiguration::getDNSServer(
    HTTPTemplateResponse *response, IPAddress::IPAddressStr &DNS1str,
...
void MTD_FLASHMEM
HTTPHelperConfiguration::setDNSServer(HTTPTemplateResponse *response) {
  ConfigurationManager::setDNSParams(
      IPAddress(response->getRequest().form[STR_DNS1]),
      IPAddress(response->getRequest().form[STR_DNS2]));
}

// must be "char dateStr[11]" and "char timeStr[9]"
void MTD_FLASHMEM HTTPHelperConfiguration::getDateTime(
    HTTPTemplateResponse *response, char *dateStr, char *timeStr) {
  // get current date
  ...
  uint8_t timezoneMinutes;
  char const *defaultNTPServer;
  ConfigurationManager::getDateTimeParams(&timezoneHours, &timezoneMinutes,
                                          &defaultNTPServer);
...
void MTD_FLASHMEM
HTTPHelperConfiguration::setDateTime(HTTPTemplateResponse *response) {
  ...
  if (tzh && tzm) {
    ConfigurationManager::setDateTimeParams(
        strtol(tzh, NULL, 10), strtol(tzm, NULL, 10), ntpsrv ? ntpsrv : STR_);
    ConfigurationManager::applyDateTime();
  }
}

void MTD_FLASHMEM
HTTPHelperConfiguration::getUART(HTTPTemplateResponse *response) {
  uint32_t baudRate;
  bool enableSystemOutput;
  SerialService serialService;
...
void MTD_FLASHMEM HTTPHelperConfiguration::getAPWiFiParams(
    HTTPTemplateResponse *response, APtr<char> &APCHStr, APtr<char> &APSECStr) {
  uint8_t channel;
  WiFi::SecurityProtocol securityProtocol;
  bool hiddenSSID;
  char const *SSID;
  char const *securityKey;
  ConfigurationManager::getAccessPointParams(&SSID, &securityKey, &channel,
                                             &securityProtocol, &hiddenSSID);
...
void MTD_FLASHMEM
HTTPHelperConfiguration::setAPWiFiParams(HTTPTemplateResponse *response) {
  ...
  if (APCH && APSEC)
    ConfigurationManager::setAccessPointParams(
        response->getRequest().form[FSTR("APSSID")],
        response->getRequest().form[FSTR("APPSW")], strtol(APCH, NULL, 10),
        (WiFi::SecurityProtocol)strtol(APSEC, NULL, 10),
        response->getRequest().form[FSTR("APHSSID")] != NULL);
}

void MTD_FLASHMEM
HTTPHelperConfiguration::getAPModeIP(HTTPTemplateResponse *response) {
  char const *IP;
  char const *netmask;
  char const *gateway;
...
void MTD_FLASHMEM
HTTPHelperConfiguration::setAPModeIP(HTTPTemplateResponse *response) {
  ConfigurationManager::setAccessPointIPParams(
      response->getRequest().form[STR_APIP],
      response->getRequest().form[STR_APMSK],
      response->getRequest().form[STR_APGTW]);
}

void MTD_FLASHMEM
HTTPHelperConfiguration::getDHCPServer(HTTPTemplateResponse *response) {
  bool DHCPDEnabled;
  char const *startIP;
  char const *endIP;
...
void MTD_FLASHMEM
HTTPHelperConfiguration::setDHCPServer(HTTPTemplateResponse *response) {
  if (response->getRequest().form[STR_DHCPD] != NULL)
    ConfigurationManager::setDHCPServerParams(
        true, response->getRequest().form[STR_startIP],
...
void MTD_FLASHMEM
HTTPHelperConfiguration::getRouting(HTTPTemplateResponse *response) {
  bool routing;
...
void MTD_FLASHMEM
HTTPHelperConfiguration::setRouting(HTTPTemplateResponse *response) {
  ConfigurationManager::setRouting(response->getRequest().form[STR_ROUTING] !=
                                   NULL);
}

// looks for "gpio" (0..16), "val" (0..1) and "store" (0..1) parameters in the
// http query
// if "store=1" then the gpio value is stored in flash
// "store" is optional
void MTD_FLASHMEM
HTTPHelperConfiguration::GPIOSetValue(HTTPResponse *response) {
  ...
  if (gpio && val) {
    ...
    if (store) {
      ...
      ConfigurationManager::getGPIOParams(gpion, &configured, &isOutput,
                                          &pullUp, &oldvalue);
      ConfigurationManager::setGPIOParams(gpion, configured, isOutput, pullUp,
                                          newvalue);
    }
...
// looks for "gpio" (0..16), "mode" (in..out), "pullup" (0..1), "store" (0..1)
// parameters in the http query
...
MTD_FLASHMEM HTTPWifiConfigurationResponse::HTTPWifiConfigurationResponse(
    HTTPHandler *httpHandler, char const *filename)
...
MTD_FLASHMEM HTTPNetworkConfigurationResponse::HTTPNetworkConfigurationResponse(
    HTTPHandler *httpHandler, char const *filename)
    : HTTPTemplateResponse(httpHandler, filename) {}
...
void MTD_FLASHMEM HTTPNetworkConfigurationResponse::flush() {
  ...
  addParamStr(STR_DISP_CLIPCONF,
              mode == WiFi::Client || mode == WiFi::ClientAndAccessPoint
                  ? STR_
                  : STR_style_display_none);

  // get access point IP configuration
  HTTPHelperConfiguration::getAPModeIP(this);
  addParamStr(STR_DISP_APIPCONF,
              mode == WiFi::AccessPoint || mode == WiFi::ClientAndAccessPoint
                  ? STR_
                  : STR_style_display_none);
...
MTD_FLASHMEM
HTTPServicesConfigurationResponse::HTTPServicesConfigurationResponse(
    HTTPHandler *httpHandler, char const *filename)
    : HTTPTemplateResponse(httpHandler, filename) {}
...
MTD_FLASHMEM HTTPWiFiScanResponseHTMLRows::HTTPWiFiScanResponseHTMLRows(
...
void MTD_FLASHMEM HTTPWiFiScanResponseHTMLRows::flush() {
  ...
  addContent(
      FSTR("<tr> <th>SSID</th> <th>Address</th> <th>Channel</th> <th>RSSI</th> "
           "<th>Security</th> </tr>"));
  for (uint32_t i = 0; i != count; ++i) {
    addContent(f_printf(FSTR("<tr> <td><input type='radio' name='selssid' "
                             "value='%s' onclick='SelSSID(this)'>%s</td> "
                             "<td>%02X:%02X:%02X:%02X:%02X:%02X</td> "
                             "<td>%d</td> <td>%d</td> <td>%s</td> </tr>"),
                        infos[i].SSID, infos[i].SSID, infos[i].BSSID[0],
                        infos[i].BSSID[1], infos[i].BSSID[2], infos[i].BSSID[3],
                        infos[i].BSSID[4], infos[i].BSSID[5], infos[i].Channel,
                        infos[i].RSSI,
...
MTD_FLASHMEM HTTPGPIOConfigurationResponse::HTTPGPIOConfigurationResponse(
    HTTPHandler *httpHandler, char const *filename)
    : HTTPTemplateResponse(httpHandler, filename) {}
...
void MTD_FLASHMEM HTTPGPIOConfigurationResponse::flush() {
  if (getRequest().method == HTTPHandler::Post) {
    ...
    if (getRequest().form[FSTR("configured")]) {
      ...
      ConfigurationManager::setGPIOParams(strtol(gpio, NULL, 10), true,
                                          f_strcmp(mode, STR_out) == 0,
                                          pullUp != NULL, false);
      ConfigurationManager::applyGPIO();
    } else if (gpio) {
      // gpio disabled (not configured)
      ConfigurationManager::setGPIOParams(strtol(gpio, NULL, 10), false, false,
                                          false, false);
    }
  }
  ...
  for (uint32_t i = 0; i != 16; ++i) {
    if (i != 1 && i != 3 && (i < 6 || i > 11)) {
      bool configured, isOutput, pullUp, value;
      ConfigurationManager::getGPIOParams(i, &configured, &isOutput, &pullUp,
                                          &value);

      linkedChunks->addChunk(
          f_printf(FSTR("<tr> <td>%d</td> <td><form method='POST'>"), i), true);
      linkedChunks->addChunk(
          f_printf(
              FSTR("Enabled <input type='checkbox' name='configured' value='1' "
                   "onclick=\"document.getElementById('GPIO%d').disabled=!this."
                   "checked\" %s>"),
              i, configured ? STR_checked : STR_),
          true);
      linkedChunks->addChunk(
          f_printf(FSTR("<fieldset class='inline' id='GPIO%d' %s>"), i,
                   configured ? STR_ : STR_disabled),
          true);
      linkedChunks->addChunk(
          f_printf(FSTR("<select name='mode'><option value='in' "
                        "%s>IN</option><option value='out' "
                        "%s>OUT</option></select>"),
      ...
      linkedChunks->addChunk(
          ...
          true);
      linkedChunks->addChunk(
          f_printf(FSTR("<input type='hidden' name='GPIO' value='%d'>"), i),
          true);
      linkedChunks->addChunk(
          FSTR("<input type='submit' value='Save'></form></td>"));
      if (configured) {
        if (isOutput) {
          linkedChunks->addChunk(
              f_printf(FSTR("<td><a href='confgpio?gpio=%d&val=%d&store=1' "
                            "class='link_button2'>%s</a></td> </tr>"),
                       i, !value, value ? STR_HI : STR_LO),
              true);
        } else {
          linkedChunks->addChunk(f_printf(FSTR("<td>%s</td> </tr>"),
                                          GPIO(i).read() ? STR_HI : STR_LO),
                                 true);
        }
      } else {
...
MTD_FLASHMEM
HTTPGPIOResponseHTML::HTTPGPIOResponseHTML(HTTPHandler *httpHandler)
    : HTTPResponse(httpHandler, NULL) {}
...
MTD_FLASHMEM HTTPTimeConfigurationResponse::HTTPTimeConfigurationResponse(
    HTTPHandler *httpHandler, char const *filename)
    : HTTPTemplateResponse(httpHandler, filename) {}
...
MTD_FLASHMEM HTTPWizardConfigurationResponse::HTTPWizardConfigurationResponse(
    HTTPHandler *httpHandler, char const *filename)
    : HTTPTemplateResponse(httpHandler, filename) {}
...
MTD_FLASHMEM HTTPFileSystemBrowserResponse::HTTPFileSystemBrowserResponse(
    HTTPHandler *httpHandler, char const *filename)
    : HTTPTemplateResponse(httpHandler, filename) {}
    ...
    linkedChunks->addChunk(
        f_printf(
            FSTR(
                ...
                "<td> "
                "<button type='button' "
                "onclick='xdelfile(\"%s\")'>Delete</button>"
                "<button type='button' onclick='openfile(\"%s\")'>Open</button>"
                "</td></tr>"),
            item.filename, item.datalength, item.mimetype, item.filename,
            item.filename),
        true);
  }
...
void MTD_FLASHMEM DefaultHTTPHandler::dispatch() {
  ...
  if (ConfigurationManager::getSerialBinary() &&
      ConfigurationManager::getSerialBinary()->isReady()) {
    StringList *routes =
        ConfigurationManager::getSerialBinary()->getHTTPRoutes();
    if (routes) {
      for (uint32_t i = 0; i != routes->size(); ++i) {
        if (f_strcmp(FSTR("*"), routes->getItem(i)) == 0 ||
            t_strcmp(getRequest().requestedPage,
                     CharIterator(routes->getItem(i))) == 0) {
          ConfigurationManager::getSerialBinary()->send_CMD_HTTPREQUEST(i,
                                                                        this);
          return;
        }
      }
