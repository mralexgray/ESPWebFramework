...
void *MTD_FLASHMEM MemPool::malloc(SIZE_T size) {
  ...
  if (freeblk) {
    // found a free block
    uint8_t *ptr = (uint8_t *)(freeblk + 1); // ptr aligned to MemPoolBlock size
                                             // (hence should be to CPU word
                                             // size)
    if (freeblk->size >
        size + sizeof(MemPoolBlock)) // is there space for another free block?
    {
      // split this block
...
void MTD_FLASHMEM MemPool::getStats(SIZE_T *largestFreeBlock,
