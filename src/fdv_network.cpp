...
char const *STC_FLASHMEM
WiFi::convSecurityProtocolToString(SecurityProtocol securityProtocol) {
...
// note: make sure there is enough stack space free otherwise mail cause reset
// (fatal exception)!
// channel: 1..13
void STC_FLASHMEM WiFi::configureAccessPoint(char const *SSID,
...
void STC_FLASHMEM WiFi::configureClient(char const *SSID,
                                        char const *securityKey) {
...
WiFi::APInfo *STC_FLASHMEM WiFi::getAPList(uint32_t *count, bool rescan,
  ...
  if (*count == 0 && canRetry) {
    // retry
    return getAPList(count, true, false); // canRetry = false, no more retry to
...
Queue<bool> *STC_FLASHMEM WiFi::getAPInfo(APInfo **infos, uint32_t *count,
                                          int32_t allocateCount) {
...
void STC_FLASHMEM IP::configureStatic(WiFi::Network network, char const *IP,
                                      char const *netmask,
                                      char const *gateway) {
  ip_info info;
...
void STC_FLASHMEM IP::getIPInfo(WiFi::Network network, uint8_t *IP,
                                uint8_t *netmask, uint8_t *gateway) {
  ip_info info;
...
void STC_FLASHMEM DHCPServer::configure(char const *startIP,
                                        char const *endIP) {
  wifi_softap_dhcps_stop();
...
IPAddress MTD_FLASHMEM NSLookup::lookup(char const *hostname) {
  ...
  addrinfo *addrinfo;
  if (lwip_getaddrinfo(memHostName.get(), NULL, NULL, &addrinfo) == 0 &&
      addrinfo) {
...
int32_t MTD_FLASHMEM Socket::read(void *buffer, uint32_t maxLength,
  ...
  sockaddr_in from;
  int fromlen;
  int32_t bytesRecv = lwip_recvfrom(m_socket, buffer, maxLength, 0,
  ...
  while (bytesSent < length) {
    ...
    int32_t chunkBytesSent =
        m_remoteAddress.sin_len == 0
            ...
            : lwip_sendto(m_socket, rambuf, bytesToSend, 0,
                          (sockaddr *)&m_remoteAddress,
...
void MTD_FLASHMEM Socket::setRemoteAddress(IPAddress remoteAddress,
                                           uint16_t remotePort) {
...
void MTD_FLASHMEM Socket::setTimeOut(uint32_t timeOut) {
  lwip_setsockopt(m_socket, SOL_SOCKET, SO_RCVTIMEO, (void *)&timeOut,
                  sizeof(timeOut));
}
...
MTD_FLASHMEM HTTPResponse::HTTPResponse(HTTPHandler *httpHandler,
                                        char const *status, char const *content)
...
void MTD_FLASHMEM HTTPResponse::flushHeaders(uint32_t contentLength) {
  if (!m_headersFlushed) {
    ...
    for (uint32_t i = 0; i != m_headers.getItemsCount(); ++i) {
      ...
      m_httpHandler->getSocket()->writeFmt(
...
void MTD_FLASHMEM HTTPResponse::flush() {
  ...
  if (m_content.getItemsCount() > 0) {
    ...
    while (chunk) {
      if (m_httpHandler->getSocket()->write((uint8_t const *)chunk->data,
                                            chunk->getItems()) < 0)
        break;
...
void MTD_FLASHMEM ParameterReplacer::start(char const *strStart,
                                           char const *strEnd, Params *params,
...
char const *MTD_FLASHMEM ParameterReplacer::replaceTag(char const *curc) {
  ...
  if (getChar(tagStart) == '#') {
    ...
    if (item)
      ...
      if (item)
        m_results.last()->addChunks(
            item->value); // push block parameter content
    }
  }
  return tagEnd + 2; // bypass "}}"
}

char const *MTD_FLASHMEM ParameterReplacer::extractTagStr(char const *curc,
                                                          char const **tagEnd) {
  char const *tagStart = curc + 2; // by pass "{{" or "{%"
  *tagEnd = tagStart;
  while (*tagEnd < m_strEnd && getChar(*tagEnd) != '}' &&
         getChar(*tagEnd) != '%')
    ++*tagEnd;
  return tagStart;
}
...
MTD_FLASHMEM
HTTPTemplateResponse::HTTPTemplateResponse(HTTPHandler *httpHandler,
                                           char const *filename)
    : HTTPResponse(httpHandler, NULL), m_filename(filename) {}

void MTD_FLASHMEM HTTPTemplateResponse::addParamStr(char const *key,
                                                    char const *value) {
  LinkedCharChunks *linkedCharChunks = m_params.add(key);
  linkedCharChunks->addChunk(value);
}

void MTD_FLASHMEM HTTPTemplateResponse::addParamInt(char const *key,
                                                    int32_t value) {
  char *valueStr = f_printf(FSTR("%d"), value);
  LinkedCharChunks *linkedCharChunks = m_params.add(key);
  linkedCharChunks->addChunk(valueStr, f_strlen(valueStr),
                             true); // true = need to free
}

void MTD_FLASHMEM HTTPTemplateResponse::addParamFmt(char const *key,
                                                    char const *fmt, ...) {
  va_list args;
  va_start(args, fmt);
  uint16_t len = vsprintf(NULL, fmt, args);
...
LinkedCharChunks *MTD_FLASHMEM
HTTPTemplateResponse::addParamCharChunks(char const *key) {
...
void MTD_FLASHMEM HTTPTemplateResponse::processFileRequest() {
  ...
  if (m_filename && FlashFileSystem::find(m_filename, &file)) {
    ...
    // replace parameters
    m_replacer.start((char const *)file.data,
                     (char const *)file.data + file.datalength, &m_params,
                     NULL);

    // is this a specialized file (contains {%..%} blocks)?
    if (m_replacer.getBlocks()->getItemsCount() > 0 &&
        m_replacer.getTemplateFilename() != NULL) {
      ...
      if (FlashFileSystem::find(m_replacer.getTemplateFilename(), &file)) {
        ...
        m_templateReplacer.start((char const *)file.data,
                                 (char const *)file.data + file.datalength,
                                 &m_params, m_replacer.getBlocks());
...
MTD_FLASHMEM UDPClient::UDPClient(IPAddress remoteAddress,
                                  uint16_t remotePort) {
...
void MTD_FLASHMEM UDPClient::init(IPAddress remoteAddress,
                                  uint16_t remotePort) {
  ...
  lwip_bind(m_socket.getSocket(), (sockaddr *)&localAddress,
            sizeof(sockaddr_in));
...
uint8_t STC_FLASHMEM ICMP::raw_recv_fn(void *arg, raw_pcb *pcb, pbuf *p,
                                       ip_addr_t *addr) {
  ICMP *this_ = (ICMP *)arg;
  ...
  if (p->tot_len >= PBUF_IP_HLEN + sizeof(icmp_echo_hdr) &&
    ...
    if (ntohs(hdr->id) == this_->m_waitingID &&
        ntohs(hdr->seqno) == this_->m_waitingSeq) {
...
err_t MTD_FLASHMEM Router::netif_input(pbuf *p, netif *inp) {
  ...
  if (ntohs(ethdr->type) == ETHTYPE_IP) {
    ...
    bool route = ((iphdr->dest.addr & inp->netmask.addr) !=
                  (inp->ip_addr.addr & inp->netmask.addr));
    // 2. check if not multicast or broadcast (>=224.0.0.0 up to
    // 255.255.255.255)
    route = route && ((iphdr->dest.addr & 0xE0) != 0xE0);

    if (route) {
      /*
      debug("netif_input intf=%d len=%d id=%d prot=%d src=%s dst=%s
      route?=%c\r\n",
...
public:
  // Sequence is: WaitingForFirstBoundary -> {BoundaryFound -> DecodingHeaders
  // -> (GettingValue | GettingFile)} -> EndOfContent
  enum States {
    WaitingForFirstBoundary,
    BoundaryFound,
    DecodingHeaders,
    GettingValue,
    GettingFile,
  ...
  FlashFile file;
  List<LinkedCharChunks> chunksFactory; // handles creation and destruction of
                                        // "headers" and "valueStorage" multiple
                                        // instances

  MultipartFormDataProcessor(char const *boundary_,
...
private:
  ...
  bool extractParameter(char const *name, CharChunksIterator curpos,
                        CharChunksIterator *nameBegin,
                        CharChunksIterator *begin, CharChunksIterator *end);
  CharChunksIterator getFilename(CharChunksIterator fullpathBegin,
                                 CharChunksIterator fullpathEnd);
};

MTD_FLASHMEM MultipartFormDataProcessor::MultipartFormDataProcessor(
    char const *boundary_, HTTPHandler::Fields *formfields_)
    : boundary(boundary_), boundarylen(f_strlen(boundary)),
      state(WaitingForFirstBoundary), substate(0), formfields(formfields_) {}

MTD_FLASHMEM bool MultipartFormDataProcessor::extractParameter(
    char const *name, CharChunksIterator curpos, CharChunksIterator *nameBegin,
    CharChunksIterator *begin, CharChunksIterator *end) {
    ...
    *nameBegin = t_strstr(curpos, CharIterator(name));
    if (*nameBegin != CharChunksIterator()) {
      ...
      *begin = it;
      ...
      while (it != CharChunksIterator() &&
             ((!hasQuote && !isspace(*it)) || (hasQuote && *it != '"')))
        ++it;
      *end = it;
...
CharChunksIterator MTD_FLASHMEM MultipartFormDataProcessor::getFilename(
    CharChunksIterator fullpathBegin, CharChunksIterator fullpathEnd) {
...
void MTD_FLASHMEM
MultipartFormDataProcessor::handle_WaitingForFirstBoundary(char c) {
  if (substate < 2) {
    // bypass first "--"
    ++substate;
...
void MTD_FLASHMEM MultipartFormDataProcessor::handle_BoundaryFound(char c) {
  ...
  static char const EOE[4] = {
      '-', '-', 0x0D, 0x0A}; // end of content (just after the ending boundary)
  if (c == EOL[substate]) {
    ++substate;
    if (substate == 2) {
      // found \r\n after boundary
...
void MTD_FLASHMEM MultipartFormDataProcessor::handle_DecodingHeaders(char c) {
  ...
  if (c == EOH[substate]) {
    ...
    if (substate == 2) {
      ...
      extractParameter(FSTR(" name="), headers->getIterator(), &keyBegin,
                       &nameBegin, &nameEnd);

      // look for "filename" parameter
      CharChunksIterator filenameBegin, filenameEnd;

      if (extractParameter(FSTR(" filename="), headers->getIterator(),
                           &keyBegin, &filenameBegin, &filenameEnd)) {
        //// this is a file
        // add "filename" to form fields
        filenameBegin =
            getFilename(filenameBegin, filenameEnd); // some browsers send a
                                                     // full path instead of a
                                                     // simple file name (IE)
        ...
        if (extractParameter(FSTR("Content-Type:"), filenameEnd, &keyBegin,
                             &contentTypeBegin, &contentTypeEnd)) {
          // add "Content-Type" to form fields
          formfields->add(keyBegin, keyBegin + 12, contentTypeBegin,
                          contentTypeEnd);

          // create file
          file.create(
              APtr<char>(t_strdup(filenameBegin, filenameEnd)).get(),
...
void MTD_FLASHMEM
MultipartFormDataProcessor::handle_GettingFile_GettingValue(char c) {
  static char const BOB[4] = {
      0x0D, 0x0A, '-', '-'}; // begin of boundary (just before every boundary)
  if (substate < 4 && c == BOB[substate]) {
    ...
    if (substate - 4 == boundarylen) {
      ...
      if (state == GettingFile)
        ...
        formfields->add(nameBegin, nameEnd, valueStorage->getIterator(),
                        CharChunksIterator());
      state = BoundaryFound;
      substate = 0;
    }
...
bool MTD_FLASHMEM HTTPHandler::processRequest() {
  ...
  CharChunksIterator headerEnd =
      t_strstr(m_receivedData.getIterator(), CharChunksIterator(),
               CharIterator(FSTR("\x0D\x0A\x0D\x0A")));
  if (headerEnd.isValid()) {
    ...
    *curc++ = 0; // ends method
    ...
    int32_t contentLength =
        contentLengthStr ? strtol(contentLengthStr, NULL, 10) : 0;
    ...
    if (m_request.method == Post) {
      ...
      if (contentType && f_strstr(contentType, FSTR("multipart/form-data"))) {
        ...
        processMultipartFormData(headerEnd, contentLength, contentType);
                 ...
                 (contentType &&
                  f_strstr(contentType,
                           FSTR("application/x-www-form-urlencoded")))) {
...
void MTD_FLASHMEM HTTPHandler::processMultipartFormData(
    CharChunksIterator headerEnd, int32_t contentLength,
  ...
  if (boundary) {
    ...
    // consume new data
    while (getSocket()->isConnected() &&
           proc.state != MultipartFormDataProcessor::EndOfContent) {
      char c;
    ...
    // dispatch must be inside this block, to have MultipartFormDataProcessor
    // content available
    dispatch();
  } else
    dispatch();
...
void MTD_FLASHMEM HTTPHandler::processXWWWFormUrlEncoded(
    CharChunksIterator headerEnd, int32_t contentLength) {
  // look for data (maybe POST data)
  if (contentLength > 0) {
    // download additional content
    int32_t missingBytes = headerEnd.getPosition() + contentLength -
                           m_receivedData.getItemsCount();
    while (getSocket()->isConnected() && missingBytes > 0) {
      int32_t bytesToRead =
          (CHUNK_CAPACITY < missingBytes ? CHUNK_CAPACITY : missingBytes);
    ...
    CharChunksIterator contentStart =
        m_receivedData
            .getIterator(); // cannot use directly headerEnd because added data
    contentStart += headerEnd.getPosition();
    extractURLEncodedFields(contentStart, CharChunksIterator(),
                            &m_request.form);
  }
...
CharChunksIterator MTD_FLASHMEM HTTPHandler::extractURLEncodedFields(
    CharChunksIterator begin, CharChunksIterator end, Fields *fields) {
...
CharChunksIterator MTD_FLASHMEM HTTPHandler::extractHeaders(
    CharChunksIterator begin, CharChunksIterator end, Fields *fields) {
  CharChunksIterator curc = begin;
...
void MTD_FLASHMEM HTTPHandler::dispatch() {
  for (uint32_t i = 0; i != m_routesCount; ++i) {
    if (f_strcmp(FSTR("*"), m_routes[i].page) == 0 ||
        t_strcmp(m_request.requestedPage, CharIterator(m_routes[i].page)) ==
            0) {
...
void MTD_FLASHMEM HTTPHandler::setRoutes(Route const *routes,
