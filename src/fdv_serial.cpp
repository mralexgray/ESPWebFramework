...
uint16_t MTD_FLASHMEM Serial::read(void *buffer, uint16_t bufferLen,
                                   uint32_t timeOutMs) {
  uint8_t *bbuf = (uint8_t *)buffer;
  uint16_t ret = 0;
  for (int16_t c; bufferLen > 0 && (c = read(timeOutMs)) > -1;
       --bufferLen, ++ret) {
    *bbuf++ = c;
  }
  return ret;
...
bool MTD_FLASHMEM Serial::readLine(bool echo, LinkedCharChunks *receivedLine,
                                   uint32_t timeOutMs) {
  while (1) {
...
void MTD_FLASHMEM HardwareSerial::reconfig(uint32_t baud_rate) {
  ...
  reg_val |= ((UART_RX_TimeOutIntrThresh & UART_RX_TOUT_THRHD)
              << UART_RX_TOUT_THRHD_S) |
             UART_RX_TOUT_EN;
  reg_val |= (UART_RX_FifoFullIntrThresh & UART_RXFIFO_FULL_THRHD)
             << UART_RXFIFO_FULL_THRHD_S;
  reg_val |= (UART_TX_FifoEmptyIntrThresh & UART_TXFIFO_EMPTY_THRHD)
...
void MTD_FLASHMEM HardwareSerial::write(uint8_t b) {
  while (true) {
    uint32_t fifo_cnt =
        READ_PERI_REG(UART_STATUS(0)) & (UART_TXFIFO_CNT << UART_TXFIFO_CNT_S);
